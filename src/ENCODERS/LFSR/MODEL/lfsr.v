...
`endif
//
module	lfsr(i_clk, i_reset, i_ce, o_word,i_din);
	parameter		WS=8,	// Number of output bits per clock
				LN=16;	// LFSR Register length/polynomial deg
	parameter [(LN-1):0]	TAPS = 16'h801C,
				INITIAL_FILL = { { (LN-1){1'b1}}, 1'b1 };
	//
	input	wire			i_clk, i_reset, i_ce;
	input   wire    [(WS-1):0]      i_din;
